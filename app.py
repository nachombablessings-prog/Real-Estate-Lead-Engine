...
st.markdown("Features: **Fuzzy Typo Correction | Multi-Page Ingestion | Algorithmic Verification**")
...
def autocorrect_location(city_input, state_input):
    raw_city = str(city_input).replace(",", "").strip().title()
    raw_state = str(state_input).replace(",", "").strip().upper()
    
    clean_state = STATE_MAPPINGS.get(raw_state, raw_state[:2] if len(raw_state) >= 2 else "TX")
    matches = difflib.get_close_matches(raw_city, KNOWN_CITIES, n=1, cutoff=0.5)
...
# --- API INGESTION ENGINE WITH QUOTA ALERTING ---
    ...
    for page in range(1, total_pages + 1):
        ...
        try:
            ...
            if response.status_code == 429 or response.status_code == 403:
                st.error("🚨 **RapidAPI Monthly Quota Exceeded / Limit Reached.** Please wait for quota reset or upgrade your plan.")
                break
            elif response.status_code != 200:
                st.warning(f"⚠️ API Notice (HTTP {response.status_code}): Could not retrieve page {page}.")
                break
...
with st.sidebar:
    ...
    input_state = st.text_input("Target State", value="TX", max_chars=10)
...
if execute_search:
    ...
    if clean_city != input_city.strip() or clean_state != input_state.strip().upper():
        st.info(f"💡 **Fuzzy Correction Applied:** Input matched to **{clean_city}, {clean_state}**.")
    ...
    with st.spinner(f"Querying {pages_to_crawl} page(s) of active listings for {clean_city}, {clean_state}..."):
        ...
        if df.empty:
            st.warning("⚠️ No qualified listings returned. Check if your API quota is active or adjust target criteria.")
        else:
                ...
                for idx, row in df.reset_index(drop=True).iterrows():
                    ...
                    with cols[col_idx]:
                        with st.container(border=True):
                            ...
                            st.markdown(f"**📍 Address:** {row['Address']}, {row['City']}, {row['State']}")
    ...
    st.info("👈 Enter your target city and state, then click **Fetch & Enrich Leads**.")
